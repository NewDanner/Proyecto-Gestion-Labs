...
public class LabsPanel extends JPanel implements DatabaseConnection.DatabaseChangeListener {
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        // Personalizar el header de la tabla (títulos de columnas)
        JTableHeader header = labsTable.getTableHeader();
        header.setFont(new Font("Arial", Font.BOLD, 14));
        header.setBackground(new Color(70, 130, 180)); // Mantener fondo azul
        header.setForeground(Color.BLACK); // Texto negro
        
        // Opcional: personalizar el renderizado de las celdas también
    ...
    private void deleteLab(int id) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            if (hasAssociatedRecords(conn, "SELECT COUNT(*) FROM Equipos WHERE id_laboratorio = ?", id, "equipos") ||
                hasAssociatedRecords(conn, "SELECT COUNT(*) FROM Prestamo WHERE Nro_Laboratorio = ?", id, "préstamos")) {
                conn.rollback();
                return;
            }

            try (PreparedStatement stmt = conn.prepareStatement("DELETE FROM Laboratorios WHERE Id_Laboratorio = ?")) {
                stmt.setInt(1, id);
                if (stmt.executeUpdate() > 0) {
                    resetAutoIncrement(conn, "Laboratorios");
                    conn.commit();
    ...
    private boolean hasAssociatedRecords(Connection conn, String query, int id, String recordType) throws SQLException {
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, id);
            ResultSet rs = stmt.executeQuery();
            if (rs.next() && rs.getInt(1) > 0) {
                if (recordType.equals("equipos") && confirmAction("Este laboratorio tiene equipos asociados. ¿Desea eliminarlos también?")) {
                    executeUpdate(conn, "DELETE FROM Equipos WHERE id_laboratorio = ?", id);
                    return false;
                }
                showError("No se puede eliminar el laboratorio porque tiene " + recordType + " asociados");
                return true;
            }
        }
        return false;
    ...
    private void resetAutoIncrement(Connection conn, String table) throws SQLException {
    ...
    private class LabDialog extends JDialog {
        ...
        public LabDialog(JFrame parent, String title) {
            ...
            cbEstado = new JComboBox<>(new String[]{"Disponible", "En mantenimiento", "Inactivo"});
            panel.add(cbEstado);
    ...
    private class EquiposDialog extends JDialog {
        ...
        private void editEquipo(ActionEvent evt) {
            ...
            EquipoForm form = new EquipoForm("Editar Equipo", new String[]{
                ...
                (String) equiposTable.getValueAt(selectedRow, 4)
            });
        ...
        private String[] getEquipoDetails(int id) {
            try (Connection conn = DatabaseConnection.getConnection();
                 PreparedStatement stmt = conn.prepareStatement(
                     "SELECT procesador, ram, almacenamiento, sistema_operativo FROM Equipos WHERE Id_Equipo = ?")) {
        ...
        private class EquipoForm extends JDialog {
            ...
            public Object[] getData() {
                return new Object[]{
                    ...
                    fields[6].getText(), cbEstado.getSelectedItem().toString().toLowerCase().replace(" ", "_"),
                    labId
                };
            }
            ...
            public Object[] getDataWithId(int id) {
                return new Object[]{
                    ...
                    cbEstado.getSelectedItem().toString().toLowerCase().replace(" ", "_"), id
                };
            }
        }
    }
