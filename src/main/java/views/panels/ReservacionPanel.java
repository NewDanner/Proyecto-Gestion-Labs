...
public class ReservacionPanel extends JPanel implements DatabaseConnection.DatabaseChangeListener {
    ...
    new SwingWorker<Void, Void>() {
        ...
        protected Void doInBackground() throws Exception {
            ...
            if (currentUser.getRole().equals("Administrador(a)")) {
                // Consulta para administradores (todas las reservas)
                queryBuilder.append("SELECT r.Id_Reserva AS id, l.nombre AS laboratorio, ")
                           .append("r.tipo_de_prestamo, r.fecha_reserva, ")
                           .append("r.hora_inicio, r.hora_fin, r.estado, ")
                           .append("u.username AS usuario, r.usa_material_adicional ")
                           .append("FROM Reservas r ")
                           .append("JOIN Laboratorios l ON r.Nro_Laboratorio = l.Id_Laboratorio ")
                           .append("JOIN Usuarios u ON r.id_usuario = u.id_usuario ")
                           .append("ORDER BY r.fecha_reserva DESC, r.hora_inicio DESC");
            } else {
                // Consulta para docentes (solo sus reservas)
                queryBuilder.append("SELECT r.Id_Reserva AS id, l.nombre AS laboratorio, ")
                           .append("r.tipo_de_prestamo, r.fecha_reserva, ")
                           .append("r.hora_inicio, r.hora_fin, r.estado, ")
                           .append("u.username AS usuario, r.usa_material_adicional ")
                           ...
                           .append("JOIN Laboratorios l ON r.Nro_Laboratorio = l.Id_Laboratorio ")
                           .append("JOIN Usuarios u ON r.id_usuario = u.id_usuario ")
                           .append("WHERE r.id_usuario = ").append(currentUser.getId()).append(" ")
                           .append("ORDER BY r.fecha_reserva DESC, r.hora_inicio DESC");
            }
